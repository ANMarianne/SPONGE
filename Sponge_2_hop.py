...
flags.DEFINE_string('dataset', 'databiggauss.npz', 'Data file path')
...
def adj_input_i(i, neigh_ind, feat, ytilde, length_scale, expo=False):
    ...
    b_i = [np.eye(num_neigh+1, number_neigh) for k in range(len(length_scale))]
    neigh_i = neigh_ind[i] 
    ind_list.append(i)
    ...
    for p in range(num_neigh+1): 
        for q in range(1, num_neigh+1):
            for k in range(len(length_scale)):
                b_i[k][p,num_neigh*p+q] = adj_(coords[ind_list[p],:], coords[ind_list[num_neigh*p+q],:], length_scale[k])#/(p+1)
        if p < num_neigh:
...
def evaluate(arange_nt, model, ind_neigh, features, y, length_scale, expo, val=False):
    """ Evaluation function.
...
def train_eval(datanum, dataname, num_valid, len_scale=[0.5], expo=False):
    ...
    features, y_tilde, y_train_test, ind_neigh, Ntrain = load_kriging_data(dataname, num_neigh, num_valid, log_y=False)
    ...
    for epoch in range(FLAGS.epochs):
        ...
        val_mse = evaluate(arange_ev, model, ind_neigh, features, y_train_test, len_scale, expo, val=True)
        ...
        if not np.logical_or(np.isnan(val_mse), np.isnan(train_loss)):
            model.save_weights("KCN_neigh_2/checkpoints"+str(datanum)+".ckpt")
          # continue
        else:
            if epoch == 0:
                break
                ...
                model.load_weights("KCN_neigh_2/checkpoints"+str(datanum)+".ckpt")
                break
    ...
    if epoch==0:
        ...
        test_mse, logits_test = evaluate(arange_te, model, ind_neigh, features, y_train_test, len_scale,\ expo)
...
def data_results(jj):
    ...
    data = np.load('content/100_datamse_mg/mediumdatagauss_'+str(jj)+'_mse.npz')
    ...
    model_egn, test_mse, tr_loss_list, val_loss_list, best_val, _ = train_eval(jj,'content/100_datamse_mg/mediumdatagauss_'+str(jj)+'_mse.npz', 25, len_scale=ls_list, expo=False)
    list_ns.append(ns)
...
#     while len(best_val_list)<9 :
    ...
    best_ranges = [10**(best_n), 10**(best_n-1), 10**(best_n-2), 10**(best_n-3)]
     ...
     with open('kcn_neigh2_mg_mse.csv', 'a+', newline='') as file:
